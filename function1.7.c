...
int soma_xulamb(int num){
	...
	while(x >= 2){
		quantidade = quantidade + comb(num);
		x--;
	}
	return quantidade;	
}
...
void tabela(FILE *input, FILE *output){
	...
	int xulambs;
	long unsigned int num;
	long unsigned int s_time, u_time, tempo;
...
/*void teste_tempo(){
	...
	srand(time(NULL));
	long unsigned int x, u_time, s_time;
	int fatores, qtd;
	
	x = 9; 
	fprintf(time_cont, "                    Contagem de tempo\n                         Fatores\nEntrada               |   Primos    |     Xulamb     |           Tempo\n");
	while (x <= 1000000000000){
		
		u_time = user_time();
		s_time = system_time();
		fatores = cont_fatores(x, lista);
		qtd = soma_xulamb(fatores, lista);
		
		fprintf(time_cont, " %-20lu |    %-6d   |      %-9d |     %-8lu     %lu \n", x, fatores, qtd, u_time, s_time);
		x = x+(rand() % x) * 10;
	}
